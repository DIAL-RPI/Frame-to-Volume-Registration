...
import torch
from train_dof_img import data_transform
import train_dof_img
import tools
import sys
...
class TestNetwork2():
    def __init__(self, case_id):
        ...
        self.landmarks = np.load('/zion/guoh9/US_recon/landmarks_gen/{}_mr.npy'.format(self.case_id))
        print('landmarks shape {}'.format(self.landmarks.shape))
        # sys.exit()
    ...
    def reg_sequence(self, seq_ids=None, adjust='0'):
        ...
        for i in range(1, seq_ids.shape[0]):
            ...
            mr_mats_gt.append(mr_mat_gt)
            mr_mats_pd.append(mr_mat_pd)
        ...
        print(best_idx_list)
        # print('gt {}, pd {}'.format(landmark_dists_gt.shape, landmark_dists_pd.shape))
        # sys.exit()
...
if __name__ == '__main__':
    ...
    # model_string = 'dcl2_big_m25_ns2'
    model_string = 'dcl2_big_m25_ns2_bi'
    ...
    model_string2 = 'dcl2_big_m25_ns2_bi_slice2'
    ...
    test_ids = tools.read_list('infos/sets/landmarks_xs_test.txt')
    ...
    for i, case_id in enumerate(test_ids[1:]):
        ...
        seq_ids = np.linspace(start, end, end-start+1).astype(np.int)
        # seq_ids = np.concatenate((seq_ids, seq_ids[::-1]))
        # print(seq_ids)
        # sys.exit()
        # seq_ids = np.asarray([50, 51])
        ...
        # # evals_1 = case.reg_sequence(seq_ids, adjust='1')
        # # np.save('tmp/evaluations/{}_{}.npy'.format(case_id, '1'), evals_1)
        # # evals_2 = case.reg_sequence(seq_ids, adjust='2')
        # # np.save('tmp/evaluations/{}_{}.npy'.format(case_id, '2'), evals_2)
        # # evals_3 = case.reg_sequence(seq_ids, adjust='3')
        # # np.save('tmp/evaluations/{}_{}.npy'.format(case_id, '3'), evals_3)
        # evals_4 = case.reg_sequence(seq_ids, adjust='4')
        # np.save('tmp/evaluations/{}_{}.npy'.format(case_id, '4'), evals_4)
        # # sys.exit()

        # seq_ids = np.linspace(0, frames_num-1, frames_num).astype(np.int)[:30]
        # seq_ids = np.linspace(0, frames_num-1, frames_num).astype(np.int)
        # case.reg_sequence(seq_ids, adjust=None)
        # case.reg_sequence(seq_ids, adjust='1')
        # sys.exit()

        # seq_ids = np.linspace(frames_num-1, 0, frames_num).astype(np.int)
        # case.reg_sequence(seq_ids)
        # sys.exit()

        # seq_ids = np.linspace(mid_id, frames_num-1, frames_num-mid_id).astype(np.int)[:30]
        # case.reg_sequence(seq_ids)
        # sys.exit()


        # seq_ids = np.linspace(mid_id, 0, mid_id+1).astype(np.int)
        # case.reg_sequence(seq_ids)

        # seq_ids1 = np.linspace(0, frames_num-1, frames_num).astype(np.int)
        # seq_ids2 = np.linspace(frames_num-1, 0, frames_num).astype(np.int)
        # seq_ids = np.concatenate((seq_ids1, seq_ids2), axis=0)
        # case.reg_sequence(seq_ids)
