...
def fuse_images(img_ref, img_float, alpha=0.4, cmap=cv2.COLORMAP_JET):
    # print(img_ref.shape)
    # print(len(img_ref.shape))
    if len(img_ref.shape) == 2:
        img_ref = np.repeat(img_ref[:, :, np.newaxis], 3, axis=2)
        # print('new {}'.format(img_ref.shape))
    
    mask = (img_float > 5).astype(np.float32)
    # print(alpha)
    mask[mask > 0.5] = alpha
    mask_comp = 1.0 - mask

    img_color = cv2.applyColorMap(img_float, cmap)
    # print(img_color.shape)

    dst = np.zeros((img_float.shape[0], img_float.shape[1], 3), dtype=np.uint8)

    for i in range(3):
        dst[:, :, i] = (img_ref[:, :, i] * mask_comp + img_color[:, :, i] * mask).astype(np.uint8)
...
def load_registration_mat2(mat_path):
    if mat_path.endswith('.xml'):
        this_mat = rr.load_registration_xml(mat_path)
        return this_mat
    elif mat_path.endswith('.txt'):
        folder_path = os.path.split(mat_path)[0]
        this_mat = rr.load_gt_registration(folder_path)
        return this_mat
    else:
        print('Registration format not supported!')
...
def load_calibration_mat(mat_path):
    if mat_path.endswith('.xml'):
        ...
        print('The calibration matrix format has not been supported yet!')
        sys.exit()
